...
/*   Updated: 2025/05/09 09:56:51 by nbodin           ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
void	fusion_quotes(char **command, char **joined, size_t i, size_t j)
{
	size_t	size;
	...
	joined[j][0] = D_QUOTE;
	joined[j][1] = 0;
	strlcat(joined[j], command[j], sizeof(joined[j]));
	strlcat(joined[j], command[j], sizeof(joined[j]));
	joined[size - 2] = D_QUOTE;
	joined[size - 1] = 0;
}

char	**join_previous_quotes(char **command, size_t i)
{
	size_t	j;
	char **joined;
